...
export default function ResultPage() {
  ...
  // Separate Men and Women results
  const menResults = categoryResults.filter((result) => result.Gender === 'Male')
  const womenResults = categoryResults.filter((result) => result.Gender === 'Female')

  // Sort by position (First, Second, Third)
  const sortByPosition = (results: ResultData[]) => {
  ...
  const sortedMenResults = sortByPosition(menResults)
  const sortedWomenResults = sortByPosition(womenResults)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFF7EB] via-[#FFF1F5] to-[#FFF7EB]">
      {/* Banner Section */}
      <section className="relative overflow-hidden py-8 md:py-12 w-full">
        <motion.div
          ...
          className="w-full"
        >
          <div className="relative w-full h-[250px] md:h-[350px] lg:h-[500px] overflow-hidden">
            <Image
              src="/result.png"
              alt="Results Banner"
              fill
        ...
        <div className="max-w-7xl mx-auto px-4">
          {isLoading ? (
            ...
            <>
              ...
              {/* Results Section */}
              {sortedMenResults.length === 0 && sortedWomenResults.length === 0 ? (
                <div className="text-center py-20">
                  <p className="text-[#640D5F] text-2xl md:text-3xl font-bold">Coming Soon</p>
                </div>
              ) : (
                <div className="space-y-12">
                  {/* Men Results Section */}
                  {sortedMenResults.length > 0 && (
                    <div>
                      <h2 className="text-2xl md:text-3xl font-bold text-[#640D5F] mb-6 text-center">
                        Men
                      </h2>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 justify-items-center max-w-7xl mx-auto">
                        {sortedMenResults.map((result, index) => (
                          <motion.div
                            key={result.Id}
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.4, delay: index * 0.1 }}
                            className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 border-2 border-[#F8C8DC] w-full max-w-sm"
                          >
                            {/* Winner Image */}
                  ...
                  {/* Women Results Section */}
                  {sortedWomenResults.length > 0 && (
                    <div>
                      <h2 className="text-2xl md:text-3xl font-bold text-[#640D5F] mb-6 text-center">
                        Women
                      </h2>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 justify-items-center max-w-7xl mx-auto">
                        {sortedWomenResults.map((result, index) => (
                          <motion.div
                            key={result.Id}
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
